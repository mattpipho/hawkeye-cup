...
$(document).ready(function () {
		...
		$.ajax({
			...
			success: function (data) {
				...
				for (i = 0; i < golfers.length; i++) {
					...
					html +=
						" <td><input type='number' name='handicap-" +
						golfers[i].id +
						"' ";
					html += "            data-mini='true'";
					html +=
						"            class='handicap' data-id='" +
						golfers[i].id +
						"'";
					html +=
						"            id='handicap-" +
						...
						"'></td>\n";
					html +=
						" <td><select name='select-golfer-team-" +
						golfers[i].id +
						"' ";
					html += "             class='select-golfer-team' ";
					html += "             data-id='" + golfers[i].id + "' ";
					html +=
						"             id='select-golfer-team-" +
						golfers[i].id +
						"' ";
					html +=
	...
	function loadRounds() {
		...
		$.ajax({
			...
			success: function (data) {
				...
				$("#select-active-round").append(
					'<option value="0">Select Round</option>'
				);
				$("#select-tee-time-round").append(
					'<option value="0">Select Round</option>'
				);
				$("#select-score-round").append(
					'<option value="0">Select Round</option>'
				);
	...
	$("#select-active-round").on("change", function () {
		...
		$.ajax({
			...
			error: function (jqXHR, textStatus, errorThrown) {
				console.log(
					"Save Active Round " + textStatus + ":" + errorThrown
				);
	...
	function refreshTeeTimes() {
		...
		$.ajax({
			...
			success: function (data) {
				//console.log("Form submitted successfully.\nReturned json: " + data["json"]);
				//console.log('teetimedata: ' + JSON.stringify(data));
				if (data["json"] != "null") {
					...
					for (i = 0; i < tee_times.length; i++) {
						...
						$(
							"#tee-time-" + tee_time_id + " .tee-time-golfer"
						).remove();

						if (
							tee_times[i].hasOwnProperty("golfers") &&
							tee_times[i].golfers != null
						) {
							...
							for (j = 0; j < numberOfGolfers; j++) {
								var golfer_id =
									tee_times[i].golfers[j].golfer_id;
								...
								var li =
									'<li class="tee-time-golfer" data-golfer-id="' +
									tee_times[i].golfers[j].golfer_id +
									'" ' +
									'    data-icon="delete"><a href="#">' +
									golfer_name +
									"</a></li>";
	...
	function refreshScores() {
		...
		$.ajax({
			...
			data: data,
			error: function (jqXHR, textStatus, errorThrown) {
				console.log("Get Scores " + textStatus + ":" + errorThrown);
			...
			success: function (data) {
				...
				if (data["json"] != "null") {
					...
					for (i = 0; i < scores.length; i++) {
						...
						html +=
							" <td>" +
							getGolferNameById(scores[i].golfer_id) +
							"</td>\n";
						var h;
						var s_out = 0;
						var s_in = 0;
	...
	$(document).on("click", ".tee-time-golfer", function (e) {
		...
		$.ajax({
			...
			error: function (jqXHR, textStatus, errorThrown) {
				console.log(
					"removeTeeTimeGolfer " + textStatus + ":" + errorThrown
				);
				$("#ajaxMessage").text("Error Saving Active Round");
				$.mobile.loading("hide");
			},
	...
	$(".select-tee-time").on("change", function () {
		if ($(this).val() != "") {
			...
			$.ajax({
				...
				error: function (jqXHR, textStatus, errorThrown) {
					console.log(
						"addTeeTimeGolfer " + textStatus + ":" + errorThrown
					);
					$("#ajaxMessage").text("Error Saving Active Round");
					$.mobile.loading("hide");
				},
	...
	$("#golferTable").on(
		...
		function () {
			...
			$.ajax({
				...
				error: function (jqXHR, textStatus, errorThrown) {
					console.log(
						"Save Golfer " + textStatus + ":" + errorThrown
					);
					$.mobile.loading("hide");
